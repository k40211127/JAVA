	...
	public static void main(String[] argsp) throws ParseException {
		...
		for (int i=0;i<Process.length;i++) {
			...
			if (count >= 0 && count <420){TimeRecord[time/60][time%60][hour][min] = startTime + nowtime+" 落在"+"Off-peak區間";}
				...
				else if (time  < 420 && count >  900 && count<1200){TimeRecord[time/60][time%60][hour][min] = startTime+"其中"+simple.format(st)+"分到7:00是 C區間,"+"7:00到15:00是B區間,15點到"+simple.format(nt)+"分是A區間";}
				else if (time  < 420 && count > 1200 && count<1320){TimeRecord[time/60][time%60][hour][min] = startTime+"其中"+simple.format(st)+"分到7:00是 C區間,"+"7:00到15:00是B區間,15點到20點是A區間,20點到"+simple.format(nt)+"分是B區間";}
				else if (time  < 420 && count > 1320 && count<1440){TimeRecord[time/60][time%60][hour][min] = startTime+"其中"+simple.format(st)+"分到7:00是 C區間,"+"7:00到15:00是B區間,15點到20點是A區間,20點到22點是B區間,22點到"+simple.format(nt)+"分是C區間";}	
				...
				else if (time  < 900 && count > 1200 && count<1320){TimeRecord[time/60][time%60][hour][min] = startTime+"其中"+simple.format(st)+"分到15:00是 B區間,"+"15:00到20:00是A區間,20點到"+simple.format(nt)+"分是B區間";}
				else if (time  < 900 && count > 1320 && count<1440){TimeRecord[time/60][time%60][hour][min] = startTime+"其中"+simple.format(st)+"分到15:00是 B區間,"+"15:00到20:00是A區間,20點到22點是B區間,22點到"+simple.format(nt)+"分是C區間";}			  
				...
				else if (time  < 1200 && count > 1320 && count<1440){TimeRecord[time/60][time%60][hour][min] = startTime+"其中"+simple.format(st)+"分到20:00是 A區間,"+"20:00到22:00是B區間,22點到"+simple.format(nt)+"分是C區間";}
